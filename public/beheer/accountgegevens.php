...
if (isset($_SESSION['auth']['user_id'])) {
    ...
    // Query om gegevens van de ingelogde klant op te halen
    $query = "SELECT 
            rol,
            email,
    ...
    $loggedInKlant = $dbManager->query($query, [':id' => $klant_id])->first();

    // Controleer of de gegevens met succes zijn opgehaald
    if ($loggedInKlant) {
        $medewerkers = [$loggedInKlant]; // Plaats de ingelogde klant in een array
    } else {
        // Als er geen gegevens zijn, initialiseer de medewerkers-array leeg
        $medewerkers = [];
    }
} else {
    // Als de klant niet is ingelogd, initialiseer de medewerkers-array leeg
    $medewerkers = [];
}
?>
...
<div class="container-lg flex-grow-1 gx-0 py-4">
    ...
    <p class="d-flex justify-content-evenly">
        ...
        <a href="accountgegevens.php" class="btn btn-secondary active">Accountgegevens</a>
